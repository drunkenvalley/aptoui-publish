...
local function CreateIcon(parent, xOffset, yOffset, iconScale, frameLayer, texturePath)
    ...
    local xSize = AptoUI.HUD.Size.Icon * iconScale
    ...
    local colours = AptoUI.Utils.GetClassColour()
...
function AptoUI.HUD.IconStrip(parent, iconCount, locationNumber, isClockwise, frameLayer, texturePath)
    local iconScale = AptoUI.HUD.GetIconChainScale(iconCount)
    ...
    local xPosition, yPosition = AptoUI.HUD.GetIconPosition(locationNumber, iconScale)
    local frames = {}
    frames[1] = CreateIcon(parent, xPosition, yPosition, iconScale, frameLayer, texturePath)
    if iconCount > 1 then
        for i = 2, iconCount, 1 do
            local xOffset, yOffset = AptoUI.HUD.GetIconOffset(offsetNumber, iconScale)
